...
- readme: pictures for readme file
...
The file **gather-data.py** aptures IMU sensor data from a DIPPID device and saves it as a CSV file for activity recognition training.
...
```
2. Output:
The Terminal will output following in this order:
Data fetching from the csv files
...
Start of training the model with the train_set.csv file
- Now lets do some training!
Start the testing of the model with the test_set.csv file
- We prepared the Model some meal (data) and fed it. Now lets test if we actually fed it right
Display of accuracy result
- and the accuracy is *drumroll* {accuracy in %}

### Start with Tutor Dataset

**DISCLAIMER:**
- If you want to use your dataset (csv files) COMBINED with the ones from everyone than please add your csv files into following existing folder:
```dataset```

After that follow from Step Nr.3

- If you want to use SOLELY your dataset (csv files) please follow the upcoming instructions
...
3. Go into following file:
   **activity_recognizer.py**

Follow the TODO steps in line 15-18. The lines look currently like this:
![before changes](https://github.com/UT-ITT/assignment-03-activity-recognition-marina-arne/readme/before-change_code-snippet.png
)
After the changes it should look like this:
![after changes](https://github.com/UT-ITT/assignment-03-activity-recognition-marina-arne/readme/after-change_code-snippet.png
)
...
IP Adress: ```IP Adress of your current PC/Notebook```
Port: ```5700```
...
## Gamification
We wanted to make the game fun thats why the pictures DONT display the activity you should do but rather the activity you are doing RIGHT NOW. That way the User can directly see what the Fitness Trainer thinks he is doing, since reading text while e.g. jumping is very hard (for some people)

# Sources
- understanding support vector machine: https://kopaljain95.medium.com/how-to-improve-support-vector-machine-9561ab96ed18 and https://medium.com/@ap.nattapoj_st/basic-comparison-between-randomforest-svm-and-xgboost-0e5862871175
- scikit-learn: https://scikit-learn.org/stable/ and https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestClassifier.html#sklearn.ensemble.RandomForestClassifier
- splitting dataset: https://medium.com/@gunkurnia/choosing-the-optimal-data-split-for-machine-learning-80-20-vs-70-30-0fd266710236
- pyglet (fancy stuff): https://pyglet.readthedocs.io/en/latest/modules/graphics/index.html
